...
def extraer_copa_mundo(ti):
    ...
    ti.xcom_push(key="extraer_copa_mundo", value=dffg)
...
def extraer_ranking(ti):
    ...
    for fifadf in fifadfs:
        try:
            ...
            pass
    ti.xcom_push(key="data_ranking", value=fifadf)

def transformar(ti):
    # Pull data from stack
    fifadf = ti.xcom_pull(key="data_ranking",task_ids='extraer_ranking')
    dffg = ti.xcom_pull(key="extraer_copa_mundo",task_ids='extraer_copa_mundo')
    print(fifadf)
    print(dffg)
    print("---------------")
    # Merge data
    ...
    print(dffg_fifa)
    ti.xcom_push(key="result", value=dffg_fifa)
...
def cargar(ti):
    ...
    dffg_fifa = ti.xcom_pull(key="result",task_ids='transformar')
    # Change type
